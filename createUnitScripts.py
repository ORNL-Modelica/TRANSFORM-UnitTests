...
folderPath = r'C:\Users\fig\Documents\Modelica\TRANSFORM-Library'
folderName = r'TRANSFORM'
folderRefPath = r'C:\Users\fig\Documents\Modelica\TRANSFORM-UnitTests'
...
cleanupRefRes = True   
folderNameRefResults=os.path.join(folderRefPath,simEnv)
...
scriptsPath = os.path.join(folderRefPath,'Scripts/')
...
for item in test_list:
                    ...
                    for val in line_list:
                        if param == 'Interval' or param == 'NumberOfIntervals':
                            ...
                            if param in val:
                                ...
                                break
    if 'Tolerance' not in exp_list:
        exp_list['Tolerance'] = '1e-4'
    ...
    if found_unitTests and found_equals:
        ...
        mosPath = os.path.join(folderRefPath, 'Scripts', simEnv, '\\'.join(modelSimPath.split('.')[1:]))
        mkdir_p(mosPath)
        
        mosPath_jMod = os.path.join(folderRefPath, 'Scripts', 'jModelica', '\\'.join(modelSimPath.split('.')[1:]))
...
if cleanupRefRes:
    if os.path.exists(folderNameRefResults):
        cleanupRefResults(unitTests,folderName,folderNameRefResults,simEnv)
    else:
        print('\n{} Does not exist. Skipping cleanup of reference results'.format(folderNameRefResults))
